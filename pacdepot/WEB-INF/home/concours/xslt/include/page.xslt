...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="css">
		<style type="text/css">
			...
			.tableHeader { font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 10pt; font-weight: bold; color: #FFFFFF; background-color: #0000FF}
	...
	<xsl:template match="/page/menu/item">
		<td class="menu" width="{@width}"><a href="{@href}"><xsl:choose><xsl:when test="@src"><img src="{@src}" border="0"/></xsl:when><xsl:otherwise><xsl:value-of select="@label"/></xsl:otherwise></xsl:choose></a></td>
	</xsl:template>
	...
	<xsl:template match="searchResultNavigator">
		...
		<table border="0" width="100%" style="border : 2px; border-color : #EEEEEE; 
			border-style : groove; border-collapse: collapse; border-spacing: 0px;" bgcolor="#EEEEEE">
			<tr>
				<td>
					<div class="text">
						<center>
					...
					<xsl:when test="item">
						<xsl:for-each select="item">
							<tr bgcolor="#FFFFFF" class="text"> 
								<td valign="top"><a href="/{sponsor/@username}">
	...
	<xsl:template name="subsection">
		...
		<center>
			<table cellpadding="5" width="100%" style="border : 2px; border-color : #CCCCCC; 
				border-style : groove; border-collapse: collapse; border-spacing: 0px;">
				<tr>
